...
dict SFWrapper::attr()
{
  ...
  result["ncentrality"] = _sfh.attr.pagerank_calc;
  return result;
}
...
dict SFWrapper::centrality()
{
  dict result;
  result.clear();
  uint64_t offset = 0;
  for (const auto& c : _sfh.centrality_names)
  {
    list clist;
    for (uint64_t i = 0; i < _sfh.attr.nodes; i++) {
      clist.append(_sfh.centrality_data[offset * _sfh.attr.nodes + i]);
    }
    result[c] = clist;
  }
  return result;
...
BOOST_PYTHON_MODULE(libseidrfile)
{
  ...
  .def("centrality", &SFWrapper::centrality)
